...
def run_gate(root: Path):
    root=root.resolve(); pre=pre_coding_gate(root); protected=git_changed(root); markers=unresolved(root); vals=[p.relative_to(root).as_posix() for p in sorted((root/"docs"/"intent-preservation"/"validation").glob("VAL-*.md"))]
